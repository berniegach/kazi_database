							...
							if($rowtrade[$columnnames[$c]]=='1')
							{
								...
								{  
									...
									if(array_key_exists($columnnames[$c],$allcerts))
									{
										...
										createNotificationsWarn($db, $notifications, $dateDifference, $username, $head.'_notifications', $columnnames[$c], $dateExpiry, $userid);
										if((int)$todayY>(int)$datePieces[2])
										{
											//the certificate has expired
											createNotifications($db, $username, $head.'_notifications', $columnnames[$c], $dateExpiry, $userid);
...
function createNotifications($db, $username, $table, $columnName, $dateExpiry, $userid)
{
	...
	$message=$username." certificate for ".$columnName." expired on ".$dateExpiry;
	$sql_command_check="SELECT *FROM $table WHERE userid = '$userid' AND messages = '$message'";
	$result_check = mysqli_query($db::$connection,$sql_command_check) ;
	if(!(!$result_check || mysqli_num_rows($result_check)>0))
	{
		//the notification is not there
        $sql_command_insert="INSERT INTO $table (userid,classes,messages,dateadded)"
			. " VALUES('$userid','1', '$message', '$today')";
...
function createNotificationsWarn($db, $notifications, $dateDifference, $username, $table, $columnName, $dateExpiry, $userid)
{
	...
	for($c=0; $c<count($noti_pieces); $c++)
	{
      ...
      if($c=='0' && $dateDifference>=7  && $dateDifference<9)
         {
             $message=$username." certificate for ".$columnName." will expire in 1 week, ".$dateExpiry;
			 $changed=TRUE;
          }
      //two weeks
      if($c=='1' && $dateDifference>=14  && $dateDifference<16)
         {
             $message=$username." certificate for ".$columnName." will expire in 2 weeks, ".$dateExpiry;
			 $changed=TRUE;
         }
      //one month
      if($c=='2' && $dateDifference>=30  && $dateDifference<32 )
        {
             $message=$username." certificate for ".$columnName." will expire in 1 month, ".$dateExpiry;
			 $changed=TRUE;
        }
      //three months
      if($c=='3' && $dateDifference>=90 && $dateDifference<92)
        {
             $message=$username." certificate for ".$columnName." will expire in 3 months, ".$dateExpiry;
			 $changed=TRUE;
        }
      //6 months
      if($c=='4' && $dateDifference>=180 && $dateDifference<182)
       {
             $message=$username." certificate for ".$columnName." will expire in 6 months, ".$dateExpiry;
			 $changed=TRUE;
       }
      //one year
      if($c=='5' && $dateDifference>=365 && $dateDifference<367)
       {
             $message=$username." certificate for ".$columnName." will expire in 1 year, ".$dateExpiry;
			 $changed=TRUE;
       }
	}
	if($changed)
