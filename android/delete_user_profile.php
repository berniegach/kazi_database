...
if (isset($_POST['id'])  && isset($_POST['bossid']))
    {
		...
		//clear the users position 
		$sql_command_update="UPDATE users_accounts SET company='', position='', datechanged='$today' WHERE id = $id";
		...
		if(!$result_update)
		{
			$response["message"] .= "error updating.". mysqli_error($db::$connection);
			$response["success"] = 0;
			echo json_encode($response);
		}
		$response["success"] = 1;
