...
if (isset($_POST['id']) && isset($_POST['equipmentname']))
    {
		...
		//get tradename id
		$sql_command_id="SELECT *FROM $tablename WHERE $equipmentname='1'";
		$result_id = mysqli_query($db::$connection,$sql_command_id) ;
		$row= mysqli_fetch_array($result_id);
		$equipmentid=$row['id'];
		//delete row
		$sql_command_delete="ALTER TABLE $tablename DROP COLUMN $equipmentname";
		$result_delete = mysqli_query($db::$connection,$sql_command_delete) ;
		if($result_delete)
		{
			$sql_command_delete_row ="DELETE FROM $tablename WHERE id = $equipmentid";
			$result_delete_row=mysqli_query($db::$connection,$sql_command_delete_row) ;
			if($result_delete_row)
			{
				//shift ids
				$sql_command_shift="UPDATE $tablename SET id=id-1 WHERE id>$equipmentid ";
